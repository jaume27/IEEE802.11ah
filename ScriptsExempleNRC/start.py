...
cal_use           = 0        # 0(disable) or 1(enable)
##################################################################################
# PHY Conf.
...
ps_timeout        = '1s'     # STA (timeout before going to sleep) (min:1000ms)
...
def setModuleParam():
    ...
    if strSTA() == 'STA' and int(power_save) > 0:
        ...
        if int(power_save) == 3:
            ...
            sleep_duration_arg = " sleep_duration=" + "".join(filter(str.isdigit,sleep_duration))
...
def insert_module_cmd():
    ...
    result = "insmod /home/pi/nrc_pkg/sw/driver/nrc.ko"
...
def run_common():
    ...
    print("sudo " + insert_module_cmd() + " " + insmod_arg)
    os.system("sudo " + insert_module_cmd() + " " + insmod_arg + "")
